...
/** PATCH /api/freizeit-packliste/:id (nur Jugendleitung). */
export const PATCH: RequestHandler = async ({ request, params }) => {
    ...
    try {
        ...
        const rec = await pb.collection('freizeit_packliste')
            .update(params.id!, pick(body));
        return json({ item: rec });
    } catch (e: any) {
        console.error('PATCH /api/freizeit-packliste failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};

/** DELETE /api/freizeit-packliste/:id (nur Jugendleitung). */
export const DELETE: RequestHandler = async ({ request, params }) => {
    ...
    try {
        const pb = await adminPb();
        await ensureFreizeitPackliste(pb);
        await pb.collection('freizeit_packliste').delete(params.id!);
        return json({ ok: true });
    } catch (e: any) {
        console.error('DELETE /api/freizeit-packliste failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};
