...
const FIELDS = ['freizeit', 'person_name', 'person_id', 'rolle', 'notiz',
    'sort_order'];
...
/** GET /api/freizeit-helfer?freizeit=ID -> Helfer + canEdit. */
export const GET: RequestHandler = async ({ request, url }) => {
    ...
    try {
        ...
        const items = await pb.collection('freizeit_helfer').getFullList({
            filter: `freizeit="${freizeit}"`,
            sort: 'rolle,sort_order,created',
        });
        return json({ items, canEdit: await isJugendLeitung(user) });
    } catch (e: any) {
        console.error('GET /api/freizeit-helfer failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};

/** POST /api/freizeit-helfer -> Helfer hinzufügen (nur Jugendleitung). */
export const POST: RequestHandler = async ({ request }) => {
    ...
    try {
        const pb = await adminPb();
        await ensureFreizeitHelfer(pb);
        const rec = await pb.collection('freizeit_helfer').create(pick(body));
        return json({ item: rec });
    } catch (e: any) {
        console.error('POST /api/freizeit-helfer failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};
