...
const FIELDS = ['titel', 'beschreibung', 'entfernung_km', 'link',
    'kategorie', 'sort_order'];
...
/** PATCH /api/unterkunft-ausflug/:id (nur Jugendleitung). */
export const PATCH: RequestHandler = async ({ request, params }) => {
    ...
    try {
        ...
        console.error('PATCH /api/unterkunft-ausflug failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};

/** DELETE /api/unterkunft-ausflug/:id (nur Jugendleitung). */
export const DELETE: RequestHandler = async ({ request, params }) => {
    ...
    try {
        const pb = await adminPb();
        await ensureUnterkunftAusflug(pb);
        await pb.collection('unterkunft_ausflugsziele').delete(params.id!);
        return json({ ok: true });
    } catch (e: any) {
        console.error('DELETE /api/unterkunft-ausflug failed:', e?.message || e);
        return json({ error: e?.message || 'Fehler' }, 500);
    }
};
