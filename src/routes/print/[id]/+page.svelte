<script lang="ts">
  ...
  const group1 = RAW_PREACHERS.filter(
    (p) => p.role === "Leiter" || p.role === "Teilnehmer",
  )
  ...
  const title =
    `PREDIGERPLAN ${format(selectedMonth, "MMMM", { locale: de })} - ${format(addMonths(selectedMonth, 1), "MMMM yyyy", { locale: de })}`.toUpperCase();
...
<div class="bg-white">
  <PrintLayout {title} {slots} {group1} {group2} {gridData} {specialEvents} />
</div>
