...
export async function loadEditorData(pb: PocketBase, user: any, planId: string) {
    ...
    //  - Mehrteilige Termine IMMER, an JEDEM Wochentag (siehe isMultiPart).
    ...
    // „Mehrteilig" = derselbe Titel kommt am selben Tag mehrfach vor (z. B.
    // „Tauffest" 08:00/09:30/14:00) ODER der Titel trägt einen „Teil"-Marker.
    const normTitle = (l: string) => (l || '').trim().toLowerCase();
    const titleDayCount = new Map<string, number>();
    for (const s of slots) {
        const k = `${s.date}|${normTitle(s.label)}`;
        titleDayCount.set(k, (titleDayCount.get(k) || 0) + 1);
    }
    ...
    // Sondergemeinschaften (Kalender 90) als VOLLSTÄNDIGE Liste fuer den Plan –
    // als Legende zu den Spalten-Sternen. Enthält ALLE echten Gemeinschafts-
    // termine (auch die mit eigener Spalte), aber OHNE Gemeindestunde (regulärer
    // Dienst) und OHNE Hochzeiten. Duplikate (gleiches Datum+Titel) entfernt.
    const seenSg = new Set<string>();
    const specialFellowships = slots
        .filter((s: any) => s.calendarId === 90)
        .filter((s: any) => {
